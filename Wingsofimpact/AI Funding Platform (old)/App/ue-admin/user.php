...
<table class="multiDataViewTable" width="100%">
        <tr>
            ...
            <td>
            <a width="100" href="<?php echo pageParamsFormat($pageParamsArr,array('orderwhat' => $namaTableDatabase.'_email', 'orderby' => ueColumnSort()))?>">
                Email
            </a>
            </td>
			<td width="100">
    ...
    ?>
        <tr<?php $rowMode = $currentRowNumber % 2; if($rowMode != 1) { echo ' class="even"'; } else { echo ' class="odd"'; } ?>>
            ...
            <td>
                <label for="<?php echo $mainQueryRes["$namaTableDatabase".'_id']?>"><?php echo wordlimiter(nohtml($mainQueryRes["$namaTableDatabase".'_email']),120)?></label>
            </td>
			<td>
                <label for="<?php echo $mainQueryRes["$namaTableDatabase".'_id']?>"><?php echo $mainQueryRes["$namaTableDatabase".'_membershiptype']?></label>
            </td>
			<td>
                <label for="<?php echo $mainQueryRes["$namaTableDatabase".'_id']?>"><?php echo $aolMembership[$mainQueryRes["$namaTableDatabase".'_membershiptype']][$mainQueryRes["$namaTableDatabase".'_membershipid']]['name']?></label>
            </td>
			<td>
                <label for="<?php echo $mainQueryRes["$namaTableDatabase".'_id']?>">
				<?php 
					if($mainQueryRes["$namaTableDatabase".'_expiry'] < time()){
					?>
						<a href="action-statusswitch.php<?php echo $pageParams?>&targetdatabasename=user&targetcolumnname=user_expiry&targetcolumnid=user_id&id=<?php echo $mainQueryRes["$namaTableDatabase".'_id']?>&frompage=<?php echo $namaPageUtama?>&action=e"><span class="purchaseStatusEmblem purchaseStatusRejected">Expired</span></a>
					<?php
					}else{
					?>
						<a href="action-statusswitch.php<?php echo $pageParams?>&targetdatabasename=user&targetcolumnname=user_expiry&targetcolumnid=user_id&id=<?php echo $mainQueryRes["$namaTableDatabase".'_id']?>&frompage=<?php echo $namaPageUtama?>&action=d"><span class="purchaseStatusEmblem purchaseStatusApproved">Active</span></a>
					<?php
					}
				?>
				</label>
			...
			<td>
                <label for="<?php echo $mainQueryRes["$namaTableDatabase".'_id']?>">
				<?php 
				if($mainQueryRes["$namaTableDatabase".'_membershipid'] == "0"){
					echo "-";
				}else{
					if($mainQueryRes["$namaTableDatabase".'_expiry'] == "0"){
						echo "-";
					}else{
						echo date("Y-m-d",$mainQueryRes["$namaTableDatabase".'_expiry']);
					}
				}
				?>
				</label>
