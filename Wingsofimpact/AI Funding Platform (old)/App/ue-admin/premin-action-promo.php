<?php
	...
	//Pattern Check START
	$promoCodeExistQue = ue_query("SELECT * FROM ".$namaTableDatabase." WHERE ".$namaTableDatabase."_code = '".$auPostedElements['code']."' AND ".$namaTableDatabase."_id != '".$id."' LIMIT 1");
	if(@ue_num_rows($promoCodeExistQue) > 0) {
		$errorList[] = 'Voucher Code Already Exist';
	}
	...
	if($auPostedElements['type'] != 2) {
		$auPostedElements['onetimeuser'] = '';
	}
	
	if($auPostedElements['mode'] != 2 && $auPostedElements['mode'] != 3) {
		$auPostedElements['maxvalue'] = 0;
	}
