...
<div class="transFullWidthWrapper" id="ueFormContainer">
    ...
    <form enctype="multipart/form-data" method="post" action="action-promo.php">
    	...
    	<table id="detailPageTable" width="100%">
        	...
        	<tr>
            	<td>
                	<?php echo $pageTitle?> Used Times
                </td>
                <td align="center">
                	:
                </td>
                <td>
                	<input type="text" readonly="readonly" disabled="disabled" value="<?php echo $detailmodeeditRes["$namaTableDatabase".'_used']?>" />
                </td>
            </tr>
            <?php
				}
        	...
        	<tr>
            	<td>
                	<?php echo $pageTitle?> Code*
                </td>
                <td align="center">
                	:
                </td>
                <td>
                	<?php echo ueCreateInputText('code','',$detailmodeeditRes["$namaTableDatabase".'_code'])?>
                </td>
            </tr>
        	<tr>
            	<td>
                ...
                <td>
					...
					<div style="float: right;" id="oneuseperuserDiv" <?php echo tooltip('Only <span class="bold">ONE</span> use per <span class="bold">Registered User</span>'); ?>>
						<label style="margin-right: 10px;">
							<input type="checkbox" name="onetimeuser" id="onetimeuser" value="e"<?
								if($detailmodeeditRes["$namaTableDatabase".'_onetimeuser'] != '') {
        	...
        	<tr class="promoLimits freeShipOff" id="maxDiscValueRow">
            	<td>
                	<?php echo $pageTitle?> Max Discount Value*
                </td>
                <td align="center">
                	:
                </td>
                <td>
                	<?php echo ueCreateInputText('maxvalue','0',$detailmodeeditRes["$namaTableDatabase".'_maxvalue'])?>
                </td>
            </tr>
...
<script type="text/javascript">
	function promoLimitOneUser() {
		curTypePromo = $('input[name="type"]:checked').val();
		if(curTypePromo == 2) {
			$('#oneuseperuserDiv').css('visibility','visible');
		}
		else {
			$('#oneuseperuserDiv').css('visibility','hidden');
		}
	}
	...
	function checkType() {
		...
		else if(curModeSel != 2 && curModeSel != 3) {
			$('#maxDiscValueRow').css('display','none');
		}
	}
