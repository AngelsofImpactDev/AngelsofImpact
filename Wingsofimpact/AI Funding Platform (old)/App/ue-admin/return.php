<?php
	...
	$namaHalamanEdit	= 'detail-return.php';
	$namaFolderUpload	= '';
...
<table class="multiDataViewTable" width="100%">
    ...
    <?php
        ...
        if($_GET['orderwhat'] != '') {
            ...
            if($_GET['orderby'] == 'asc') {
                $mainQueryString = $mainQueryString." ORDER BY ".$namaTableDatabase."_entrydate ASC";
            }
            else {
        ...
        <tr<?php $rowMode = $currentRowNumber % 2; if($rowMode != 1) { echo ' class="even"'; } else { echo ' class="odd"'; } ?>>
            ...
            <td <?php
				if($mainQueryRes["$namaTableDatabase".'_image']) {
            		echo tooltip('<img src="../upload/bankLogo/'.$mainQueryRes["$namaTableDatabase".'_image'].'" width="100" />',100);
				}
			?>>
