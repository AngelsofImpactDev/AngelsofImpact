<?php
	...
	if(!is_numeric($auPostedElements['amount'])) {
		$errorList[] = 'Amount must be number only without formatting';
	}
	//Pattern Check END
		...
		switch($detailmode) {
			case 'edit':
				$editQueryStr	= "UPDATE ".$namaTableDatabase." SET
									...
									".$namaTableDatabase."_deadline = '".strtotime($auPostedElements['deadline'])."',
									".$namaTableDatabase."_repaymentstart = '".strtotime($auPostedElements['repayment'])."',
									".$namaTableDatabase."_amount = '".$auPostedElements['amount']."',
									".$namaTableDatabase."_active = '".$auPostedElements['status']."'
		...
		switch($detailmode) {
			case 'edit':
				if($grandQueryResult == false) {
					...
					$sql= "SELECT a.*, b.user_name as investorName, b.user_email as investorEmail, c.startup_name, c.startup_repaymentperiod, c.startup_repaymentstart, d.user_name as seName, d.user_email as seEmail FROM ".$namaTableDatabase." a 
					...
					$messageQuery 		= "Dearest ".$rs['investorName'].",
					...
					$messageQuery 		= "Dearest ".$rs['seName'].", $nl
