<div class="transFullWidthWrapper centerText">
	<?php
		if($_GET['gTrendsMonthToCountry'] == 'Worldwide') {
			$gTrendsMonthToCountry = '';
		}
		else {
			$gTrendsMonthToCountry = 'ID'; //Leave '' For Worldwide
		}
		
		if($_GET['gTrendsMonthToCheck']) {
			$gTrendsMonthToCheck = $_GET['gTrendsMonthToCheck'];
		}
		else {
			$gTrendsMonthToCheck = '6';
		}
		
		//Keywords Reformatting START
		if($_GET['gTrendsKeywords']) {
			$gTrendsKeywords = $_GET['gTrendsKeywords'];
		}
		else {
			$gTrendsKeywords = $globvar_keywords;
		}
		$gTrendsKeywords = str_replace(', ',',',$gTrendsKeywords);
		$gTrendsKeywords = str_replace(' ','+',$gTrendsKeywords);
		$gTrendsKeywordsArr = explode(',',$gTrendsKeywords);
		$gTrendsKeywordsFirst = '';
		$gTrendsKeywordsRes = '';
		$gTrendsKeywordsNum = 0;
		foreach($gTrendsKeywordsArr as $gTrendsKeywordsArrKey => $gTrendsKeywordsVal) {
			$gTrendsKeywordsRes .= $gTrendsKeywordsVal.',';
			$gTrendsKeywordsNum++;
			if($gTrendsKeywordsNum == 5) {
				break;
			}
			if($gTrendsKeywordsNum == 1) {
				$gTrendsKeywordsFirst = $gTrendsKeywordsVal;
			}
		}
		$gTrendsKeywordsRes = substr($gTrendsKeywordsRes,0,-1);
		//Keywords Reformatting END
		...
		<tr>
			...
			<td class="rightText smallSelect">
				<select name="gTrendsMonthToCountry" id="gTrendsMonthToCountry">
					...
					<?php
						foreach($countryList as $countryListKey => $countryListVal) {
					?>
					<option value="<?php echo $countryListKey ?>" <?php
						if($gTrendsMonthToCountry == $countryListKey) {
							echo 'selected="selected"';
						}
					?> class="leftText"><?php echo $countryListVal ?></option>
					<?php
						}
					?>
				</select>
				<select name="gTrendsMonthToCheck" id="gTrendsMonthToCheck">
					...
					<option value="<?php echo $i ?>" <?php
						if($gTrendsMonthToCheck == $i) {
							echo 'selected="selected"';
						}
					?> class="leftText">Last <?php echo $i ?> Months</option>
					<?php 
						}
					?>
				</select>
	...
	<script type="text/javascript">
		$('#gTrendsMonthToCountry,#gTrendsMonthToCheck').change(function() {
			$('#gTrendsForm').submit();
		});
	</script>
	<table width="100%" cellpadding="0" cellspacing="0">
